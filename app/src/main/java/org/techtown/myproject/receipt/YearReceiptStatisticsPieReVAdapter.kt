...
class YearReceiptStatisticsPieReVAdapter(val receiptPieModel : ArrayList<ReceiptPieModel>):
    RecyclerView.Adapter<YearReceiptStatisticsPieReVAdapter.ReceiptStatisticsPieViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ReceiptStatisticsPieViewHolder, position: Int) {
        ...
        holder.priceArea!!.text =
            decimalFormat.format(receiptPieModel[position].price.replace(",", "").toDouble()) + "원"
    }

    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): ReceiptStatisticsPieViewHolder {
        val view = LayoutInflater.from(parent!!.context)
            .inflate(R.layout.receipt_pie_list_item, parent, false)
