...
class SearchQuestionFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val v : View? = inflater.inflate(R.layout.fragment_search_question, container, false)
        ...
        communityListView.setOnItemClickListener { _, _, position, _ ->
            ...
            intent.putExtra("category", "질문") // 게시물의 카테고리 넘기기
                        ...
                        when (category) {
                            ...
                            "작성자" -> {
                                val userName = FBRef.userRef.child(item!!.uid).child("nickName").get().addOnSuccessListener {
                                    Log.d("nickName", it.value.toString() + " " + searchText)
                                    if(it.value.toString().contains(searchText)) {
                                        if(date == "전체") { // 날짜가 전체일 경우
                                            var nowDateSp = item!!.time.split(" ")
                                            var dateSp = nowDateSp[0].split(".")
                                            var timeSp = nowDateSp[1].split(":")
