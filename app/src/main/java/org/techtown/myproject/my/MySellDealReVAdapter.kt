...
class MySellDealReVAdapter(val dealList : ArrayList<DealModel>):
    RecyclerView.Adapter<MySellDealReVAdapter.MySellDealViewHolder>() {
    ...
    override fun onBindViewHolder(holder: MySellDealViewHolder, position: Int) {
        val myUid = FirebaseAuth.getInstance().currentUser?.uid.toString() // 현재 로그인된 유저의 ui
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MySellDealViewHolder {
        val view = LayoutInflater.from(parent!!.context).inflate(R.layout.my_sell_deal_list_item, parent, false)
        return MySellDealViewHolder(view)
    }

    inner class MySellDealViewHolder(view : View?) : RecyclerView.ViewHolder(view!!) {
        val view = view
