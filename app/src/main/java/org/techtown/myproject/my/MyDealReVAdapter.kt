...
import org.techtown.myproject.deal.DealReVAdapter
...
class MyDealReVAdapter(val dealList : ArrayList<DealModel>):
    RecyclerView.Adapter<MyDealReVAdapter.MyDealViewHolder>() {
    ...
    override fun onBindViewHolder(holder: MyDealViewHolder, position: Int) {
        ...
        when(dealList[position].state) {
            "예약 중" ->  {
                holder!!.reservationArea!!.visibility = VISIBLE
                holder!!.completeArea!!.visibility = GONE
            }
            "거래 완료" -> {
                holder!!.reservationArea!!.visibility = GONE
                holder!!.completeArea!!.visibility = VISIBLE
            }
            else -> {
                holder!!.reservationArea!!.visibility = GONE
                holder!!.completeArea!!.visibility = GONE
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyDealViewHolder {
        val view = LayoutInflater.from(parent!!.context).inflate(R.layout.my_deal_list_item, parent, false)
        return MyDealViewHolder(view)
    }

    inner class MyDealViewHolder(view : View?) : RecyclerView.ViewHolder(view!!) {
        val view = view
        ...
        val reservationArea = view?.findViewById<TextView>(R.id.reservationArea)
        val completeArea = view?.findViewById<TextView>(R.id.completeArea)
